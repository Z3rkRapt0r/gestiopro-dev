-- ============================================================================
-- FIX HTTP EXTENSION SCHEMA IN ATTENDANCE MONITOR CRON
-- ============================================================================
-- Fixes "function http_header(unknown, unknown) does not exist" error
-- by including 'extensions' schema in search_path
-- ============================================================================
...
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, extensions, pg_temp  -- 🔧 Added 'extensions' schema
AS $$
DECLARE
    ...
    BEGIN
        ...
        LOOP
            ...
            LOOP
                ...
                -- Determina se è un giorno lavorativo usando colonne booleane
        ...
        IF alerts_created > 0 THEN
            ...
            BEGIN
                -- 🔧 FIX: Cast espliciti per http_header e qualificazione completa
                SELECT content INTO edge_response
                FROM http((
                    'POST'::text,
                    'https://nohufgceuqhkycsdffqj.supabase.co/functions/v1/attendance-monitor'::text,
                    ARRAY[
                        http_header('Content-Type'::varchar, 'application/json'::varchar),
                        http_header('Authorization'::varchar, 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im5vaHVmZ2NldXFoa3ljc2RmZnFqIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDk4OTEyNzYsImV4cCI6MjA2NTQ2NzI3Nn0.oigK8ck7f_sBeXfJ8P1ySdqMHiVpXdjkoBSR4uMZgRQ'::varchar)
                    ],
...
COMMENT ON FUNCTION public.attendance_monitor_cron() IS
  'Monitors employee attendance with proper extension schema access for HTTP calls';

-- Test
SELECT '✅ Fix HTTP extension schema applicato!' as status;
