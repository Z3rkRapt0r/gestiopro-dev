-- Soluzione finale: Solo cron job interno
-- Il cron job identifica gli avvisi e li inserisce in attendance_alerts
-- L'Edge function send-attendance-alerts può essere chiamata manualmente o tramite webhook
...
-- Funzione finale per schedulare gli avvisi
...
BEGIN
    ...
    RAISE NOTICE 'Starting attendance check: % admins enabled, % employees active, % pending alerts, day: %', 
    ...
    RAISE NOTICE 'Attendance check completed: % new alerts created, % total pending', alerts_created, pending_alerts;
    
    RETURN 'Check completed at ' || current_timestamp_val || 
           '. New alerts created: ' || alerts_created || 
           ' | Total pending: ' || pending_alerts;
END;
$$;

-- Configura il cron job finale
SELECT cron.schedule(
    'attendance-alerts-only',
    '*/15 * * * *',
...
SELECT 
    '✅ Final cron job creato con successo' as status,
    jobid, 
    schedule, 
    jobname,
    active
FROM cron.job 
WHERE jobname = 'attendance-alerts-only';
