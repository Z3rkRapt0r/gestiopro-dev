-- SOLUZIONE COMPLETA: Cron job + Edge function automatica
-- Questo script crea un sistema che identifica gli avvisi e chiama automaticamente l'Edge function
...
-- Abilita l'estensione http per chiamare le Edge functions
...
-- Funzione COMPLETA che identifica avvisi e chiama l'Edge function
CREATE OR REPLACE FUNCTION public.completo_attendance_check()
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- Configura il cron job completo
SELECT cron.schedule(
    'completo-attendance-check',
    '*/15 * * * *',
    'SELECT public.completo_attendance_check();'
);

-- Verifica che il cron job sia stato creato
SELECT 
    '✅ Cron job completo creato con successo' as status,
    jobid, 
    schedule, 
    jobname,
    active
FROM cron.job 
WHERE jobname = 'completo-attendance-check';

-- Test immediato della funzione completa
SELECT public.completo_attendance_check();
