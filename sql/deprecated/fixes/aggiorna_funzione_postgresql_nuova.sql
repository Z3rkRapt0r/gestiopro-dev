-- =========================================
-- 🔄 AGGIORNA FUNZIONE POSTGRESQL PER NUOVA EDGE FUNCTION
-- =========================================
...
BEGIN
    ...
    -- Se ci sono avvisi, chiama la NUOVA Edge Function send-attendance-notifications
    IF alerts_created > 0 OR pending_alerts > 0 THEN
        RAISE NOTICE 'Chiamata NUOVA Edge function send-attendance-notifications per inviare % avvisi pendenti', alerts_created + pending_alerts;

        BEGIN
            -- Chiama la NUOVA Edge Function send-attendance-notifications
            ...
            RAISE NOTICE 'Edge function send-attendance-notifications risposta: %', edge_response;
            edge_response := COALESCE(edge_response, 'Risposta vuota');

        EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE 'Errore chiamata Edge function send-attendance-notifications: %', SQLERRM;
