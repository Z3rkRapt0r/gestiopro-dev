-- =========================================
-- ✅ VERSIONE FINALE - FIX FOREIGN KEY
-- =========================================
...
BEGIN
    ...
    -- FIX: Ciclo per ogni amministratore abilitato CHE ESISTE ANCHE IN PROFILES
    FOR admin_record IN
        ...
        JOIN profiles p ON a.id = p.id  -- JOIN per garantire che l'admin esista in profiles
        WHERE a.attendance_alert_enabled = true
        AND p.is_active = true  -- Solo admin attivi
        AND p.role IN ('admin', 'super_admin')  -- Solo ruoli admin
    LOOP
        ...
        LOOP
            ...
            -- FIX: Verifica se è in ferie/permesso oggi usando user_id e date_from/date_to
            ...
            -- FIX: Verifica se ha già fatto il check-in oggi usando user_id
            SELECT COUNT(*) INTO alert_count
            FROM attendances
    ...
    IF alerts_created > 0 OR pending_alerts > 0 THEN
        ...
        END;
    ELSE
        RAISE NOTICE 'Nessun avviso da creare o da inviare';
