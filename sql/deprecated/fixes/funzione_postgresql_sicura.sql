-- =========================================
-- 🔒 FUNZIONE POSTGRESQL SICURA E AGGIORNATA
-- =========================================
...
DECLARE
    ...
    valid_admin_found boolean := false;
BEGIN
    -- Inizializzazione
    current_day_name := lower(trim(to_char(current_timestamp_val, 'Day')));
    current_date_str := to_char(current_timestamp_val, 'YYYY-MM-DD');
    current_time_str := to_char(current_timestamp_val, 'HH24:MI:SS');

    -- Conta amministratori abilitati E CON IMPOSTAZIONI COMPLETE
    ...
    AND a.resend_api_key IS NOT NULL
    AND a.sender_email IS NOT NULL;

    -- Se non ci sono amministratori validi, termina
    IF admin_count = 0 THEN
        result_message := 'Nessun amministratore abilitato con impostazioni complete (API key ed email mittente). Controllo saltato.';
    ...
    RAISE NOTICE 'Controllo presenze: % admin validi, % dipendenti attivi, giorno: %',
        admin_count, total_employees, current_day_name;

    -- Controllo globale degli orari aziendali
    ...
    IF company_start_hour IS NULL OR company_end_hour IS NULL THEN
        company_start_hour := 8;
        company_end_hour := 17;
        RAISE NOTICE 'Orari aziendali di default: %:00 - %:00', company_start_hour, company_end_hour;
    END IF;
    ...
    -- Verifica se dobbiamo controllare oggi
    ...
    -- Ciclo per ogni amministratore VALIDO (con impostazioni complete)
    FOR admin_record IN
        SELECT a.id as admin_id, a.attendance_alert_delay_minutes, a.resend_api_key, a.sender_name, a.sender_email
        FROM admin_settings a
        JOIN profiles p ON a.id = p.id
        WHERE a.attendance_alert_enabled = true
        AND p.is_active = true
    ...
    LOOP
        ...
        LOOP
            ...
            WHERE lr.user_id = employee_record.id
            AND current_date_str::date BETWEEN lr.date_from AND lr.date_to
            ...
            -- Verifica se avviso già creato oggi
            SELECT COUNT(*) INTO alert_count
            ...
            -- Crea avviso SOLO se admin_id è valido
    ...
    -- Se non abbiamo trovato amministratori validi
    IF NOT valid_admin_found THEN
        result_message := 'Nessun amministratore valido trovato con API key ed email configurate.';
        RAISE NOTICE '%', result_message;
        RETURN result_message;
    END IF;

    -- Conta avvisi pendenti
    SELECT COUNT(*) INTO pending_alerts
    FROM attendance_alerts
    ...
    -- Se ci sono avvisi, chiama la Edge Function
    IF alerts_created > 0 OR pending_alerts > 0 THEN
        RAISE NOTICE 'Chiamata Edge function send-attendance-notifications per % avvisi pendenti', pending_alerts;
    ...
    result_message := 'Controllo completato: ' || alerts_created || ' avvisi creati, risposta Edge: ' || edge_response;
    RAISE NOTICE '%', result_message;
