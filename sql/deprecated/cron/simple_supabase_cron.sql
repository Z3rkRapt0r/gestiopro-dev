...
-- Crea una funzione semplificata per il controllo entrate
CREATE OR REPLACE FUNCTION public.check_missing_attendance_simple()
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- Configura il cron job per eseguire il controllo ogni 15 minuti
SELECT cron.schedule(
    'check-missing-attendance',
    '*/15 * * * *',
    'SELECT public.check_missing_attendance_simple();'
);
