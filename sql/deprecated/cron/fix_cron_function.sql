-- FIX: Correggi la funzione del cron job per chiamare l'Edge function corretta

-- Aggiorna la funzione per chiamare send-attendance-alerts invece di check-missing-attendance
...
BEGIN
    ...
    BEGIN
        ...
        -- Se ci sono avvisi da inviare, chiama l'Edge function CORRETTA
        IF pending_alerts > 0 THEN
            RAISE NOTICE 'Chiamata Edge function send-attendance-alerts per inviare % avvisi', pending_alerts;
            
            BEGIN
                -- Chiama l'Edge function CORRETTA per inviare le email
                ...
                RAISE NOTICE 'Edge function send-attendance-alerts risposta: %', edge_response;
                edge_response := COALESCE(edge_response, 'Risposta vuota');
                
            EXCEPTION WHEN OTHERS THEN
                RAISE NOTICE 'Errore chiamata Edge function send-attendance-alerts: %', SQLERRM;
...
SELECT 
    '✅ Funzione corretta aggiornata' as info,
