-- Setup ibrido: Cron job interno + chiamata Edge function
-- Questo script crea un sistema che usa il cron job interno per identificare gli avvisi
-- e poi chiama l'Edge function per inviare le email
...
-- Crea la tabella per i trigger di controllo presenze
...
-- Funzione per schedulare gli avvisi (solo identificazione)
...
-- Funzione ibrida che chiama sia il controllo interno che l'Edge function
CREATE OR REPLACE FUNCTION public.hybrid_attendance_check()
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
    ...
    -- 1. Esegui il controllo interno (identifica gli avvisi)
    result := public.schedule_attendance_alerts();
    
    -- 2. Conta quanti avvisi sono stati creati
    SELECT COUNT(*) INTO alerts_count
    FROM attendance_alerts 
    WHERE alert_date = current_date_str::date 
    AND email_sent_at IS NULL;
    
    RAISE NOTICE 'Hybrid check: % alerts pending, calling Edge function', alerts_count;
    
    -- 3. Se ci sono avvisi da inviare, chiama l'Edge function
    IF alerts_count > 0 THEN
        ...
        RAISE NOTICE 'Edge function send-attendance-alerts called';
    END IF;
    
    RETURN result || ' | Edge function called for ' || alerts_count || ' alerts';
END;
$$;

-- Configura il cron job ibrido
SELECT cron.schedule(
    'hybrid-attendance-check',
    '*/15 * * * *',
    'SELECT public.hybrid_attendance_check();'
);

-- Verifica che il cron job sia stato creato
SELECT 
    '✅ Hybrid cron job creato con successo' as status,
    jobid, 
    schedule, 
    jobname,
    active
FROM cron.job 
WHERE jobname = 'hybrid-attendance-check';

-- Test immediato della funzione ibrida
SELECT public.hybrid_attendance_check();
