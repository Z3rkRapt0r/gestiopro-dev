...
export function useLoginSettings() {
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      ...
      .on('postgres_changes', 
        ...
        (payload) => {
          console.log('Dashboard settings changed, reloading...', payload);
          loadSettings();
        }
      )
      .subscribe();
  ...
  const loadSettings = async () => {
    try {
      ...
      // Carica le impostazioni più recenti ordinando per updated_at
      ...
      console.log('Login settings loaded from DB:', data);

      if (data) {
        // Se ci sono dati, usa quelli dal database
        const newSettings = {
