...
export default function ArchiveEmployeeView({
  employeeId,
  attendances,
  type
}: ArchiveEmployeeViewProps) {
  const {
    employees
  } = useActiveEmployees();
  const {
  ...
  const groupIntoPeriods = (attendanceList: UnifiedAttendance[]): AttendancePeriod[] => {
    ...
    // Sort by date
    const sortedAttendances = [...attendanceList].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
    ...
    for (let i = 1; i < sortedAttendances.length; i++) {
      ...
      const isSameType = current.is_sick_leave === previous.is_sick_leave && current.is_business_trip === previous.is_business_trip && current.is_manual === previous.is_manual;
  ...
  const formatPeriod = (period: AttendancePeriod) => {
    const startDate = format(new Date(period.startDate), 'dd/MM/yyyy', {
      locale: it
    });
    const endDate = format(new Date(period.endDate), 'dd/MM/yyyy', {
      locale: it
    });
  ...
  return <div className="space-y-6">
      ...
      <Tabs value={selectedYear} onValueChange={setSelectedYear}>
        <TabsList className="grid w-full grid-cols-4 lg:grid-cols-6">
          {Object.keys(yearGroups).map(year => <TabsTrigger key={year} value={year} className="flex items-center gap-2">
        ...
        {Object.keys(yearGroups).map(year => <TabsContent key={year} value={year} className="mt-6">
            <div className="grid gap-4">
              {yearGroups[year].length === 0 ? <Card>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {yearGroups[year].map(period => <div key={period.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              ...
                              <AlertDialogFooter>
                                ...
                                <AlertDialogAction onClick={() => handleDeletePeriod(period)} className="bg-red-600 hover:bg-red-700">
                                  Elimina Periodo
                                </AlertDialogAction>
                              </AlertDialogFooter>
                            </AlertDialogContent>
